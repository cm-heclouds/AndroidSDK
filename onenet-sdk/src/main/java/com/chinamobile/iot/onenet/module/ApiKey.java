...
public class ApiKey {
    ...
    public static String urlForAdding() {
        return new HttpUrl.Builder().scheme(Urls.SCHEME).host(Urls.HOST).addPathSegment("keys").toString();
    }

    public static String urlForUpdating(String key) {
        return new HttpUrl.Builder().scheme(Urls.SCHEME).host(Urls.HOST).addPathSegment("keys")
                .addPathSegment(key).toString();
    }

    public static String urlForQuerying(String key, int page, int perPage, String deviceId) {
        HttpUrl.Builder builder = new HttpUrl.Builder();
        builder.scheme(Urls.SCHEME).host(Urls.HOST).addPathSegment("keys");
        if (key != null) {
